...
class PREDICTOR
{
  ...
  private:
    static const int num_DNF_terms = 4; 
    struct branch_info_t
    {
	std::vector <int> DNF;   
	typedef uint8_t last_N_pred; 
	typedef uint8_t iteration; //determine if just beginning building the branch 
	typedef uint8_t num_pred_T_but_NT;
    } branch_info; 
  ...
  public:
    ...
    bool get_prediction(const branch_record_c* br, const op_state_c* os)
	    ...
	    std::cout << "pht contains:";
	    for (unsigned i=0; i < pht.size(); i++)
		std::cout << ' ' << (int)pht.at(i);
	    std::cout << '\n'; 
	    //printf("bhr: %d \n",bhr);  
